...
$$
\lambda(x)=\lim_{\Delta x\to 0} \frac{P(x\le X< {x+\Delta x}|X \ge x)}{\Delta x}. 
$$ 
...
$$
P(x\le X< x+\Delta x|X \ge x) = \frac{P(x\le X< x+\Delta x \cap X \ge x)}{P(X \ge x)} \approx \frac{f(x)\Delta x}{S(x)}
$$

Therefore

$$
\lambda(x)=\lim_{\Delta x\to 0} \frac{P(x\le X< {x+\Delta x}|X \ge x)}{\Delta x} \approx \frac{f(x) \Delta x/S(x)}{\Delta x}= \frac{f(x)}{S(x)}. 
$$ 


$\lambda (x)$ is also called failure rate function.
...
* **Consistent estimator** of the true CDF, following the Glivenko–Cantelli Theorem. As more data are collected, the ECDF becomes arbitrarily close to the true CDF **uniformly across** all points on the real line, i.e., $\sup_x|F_n(x)−F(x)|\to 0$ almost everywhere.

* Step function with jumps at observed data points

* Asymptotically Unbiased estimator: $\lim_{n \to \infty} E[\hat{F}_n(x)] = F(x)$.


<font color = "red">**Because of these favorable properties, $F_n(x)$ serves as a reference estimator for comparing other estimators of an unknown $F(x)$ based on a given data set.**</font>
...
The following animated chart shows the process of FDE with five points.


```{r echo = FALSE, fig.align='center', out.width="80%"}
include_graphics("KDE.gif")
```
