...
cat("  True parameters: μ =", true_mu, ", σ =", true_sigma, "\n",
 "   MoM estimates: μ_hat =", round(mom_mu, 3), 
...
cat("  True parameters: α =", true_alpha, ", β =", true_beta, "\n",
     "   MoM estimates: α_hat =", round(mom_alpha, 3), 
...
Recall that the equation to solve is

$$
g(k) = \frac{\Gamma\left(1 + \frac{2}{k}\right)}{\left[\Gamma\left(1 + \frac{1}{k}\right)\right]^2} - \frac{m_2}{m_1^2} 
$$

Next we use simulated data from the Weibull distribution with $k = 2.5$ and $\lambda = 3$.



## Using `uniroot()`

The `uniroot()` function in R finds the root (zero) of a continuous function over a specified interval. Using Brent's algorithm, it requires the function to take opposite signs at the interval endpoints—ensuring at least one root lies within. Since $g(k)$ is a single variable function, we can plot the function and identify the interval visually.
...
# refer to the last formula in Section 4, the g() must be written as a function of k
# in the following. k is the shape parameter

gk <- function(k){
  (gamma(1+2/k) / ((gamma(1+1/k))^2)) - m2/m1^2
}

## identify end values of the interval that contains the root
## Since this a single variable function, we can curve the function
## and then find the interval

# Create a data frame with x values and corresponding y values
x_vals <- seq(1, 5, length.out = 50)
dframe <- data.frame(x = x_vals, y = gk(x_vals))
##
gk.plot <- ggplot(dframe, aes(x = x, y = y)) +
  ...
  labs(title = "The curve of function g(k)",
       x = "k", 
       y = "g(k)") +
...
As the curve above indicates the root lies in (2, 3), we now call uniroot() in R to solve $g(k) = 0$ and obtain the MME of the scale parameter $\lambda$.

```{r}
## uniroot() to find k
k <- uniroot(gk, interval = c(2, 3))$root
## find the scale parameter
## refer to the equation in step 4
lambda <- m1/gamma(1+1/k)
pander(cbind(k = k, lambda = lambda))
```
...
The R function `optimize()` is used for one-dimensional optimization—finding the minimum or maximum of a function over a specified interval. However, it requires an objective function, denoted by $f(k)$, such that the solution to $f'(x) = 0$ is equivalent to $g(x) = 0$. One such choice is to let $f(k) = [g(k)]^2$. Note that $g'(k) < 0$ (as seen from the curve of $g(k)$ above).

$$
f'(k) = 2g'(k)g(k) = 0 \quad  \text{is equivalent to} \quad g(k) = 0
$$

In the following, we will use $f(k)$ as the objective function and employ optimize() to find the root of $g(k) = 0$.
